...
const insertProduct = async (product) => {
    try{
        ...
        const query = await getDB().collection('products').insertOne(productDetails);
...
const compareProduct = async (product) => {
    try {
        ...
        console.log(`-> Found${res.data.data && res.data.data.length === 50 ? ` at least` : ``} ${res.data.data && res.data.data.length} cheaper items with the comparison API\n`);
...
const findCheaperLocalProduct = async (product) => {
    console.log('-> Now Looking for cheapert local items...\n');
    try{
        const query = parser(`{"cloudiqProductPrice": {$lt : ${product.cloudiqProductPrice}}}`);
        const result = await getDB().collection('products').findOne(query);
        if(result) {
            console.log('-> Found!\n');
